...
public class Calculator {
    ...
    private Double prepareParam(String str) {
        ...
        // 长度校验
        if (str.length() > ArithmeticUtils.FORMAT_MAX_LENGTH) {
        ...
        // 校验格式
        if (!ArithmeticUtils.checkFormat(str)) {
            System.out.println("表达式错误！");
            return null;
        }
        // 处理表达式，改为标准表达式
        str = ArithmeticUtils.change2StandardFormat(str);
        // 拆分字符和数字
    ...
    private Double doCalculate(String symStr, List<Double> numLst) {
        ...
        while (symStack.isEmpty() || !(symStack.getLast() == '=' && symStr.charAt(j) == '=')) {// 形如：
            ...
            if (ArithmeticUtils.symLvMap.get(symStr.charAt(j)) > ArithmeticUtils.symLvMap.get(symStack.getLast())) {// 比较符号优先级，若当前符号优先级大于前一个则压栈
                if (symStr.charAt(j) == '(') {
                ...
                switch (sym) {
                    case '+':
                        numStack.add(ArithmeticUtils.plus(num1, num2));
                        break;
                    case '-':
                        numStack.add(ArithmeticUtils.reduce(num1, num2));
                        break;
                    case '*':
                        numStack.add(ArithmeticUtils.multiply(num1, num2));
                        break;
                    case '/':
                        if (num2 == 0) {// 除数为0
                        ...
                        numStack.add(ArithmeticUtils.divide(num1, num2));
                        break;
                }
            }
        }
