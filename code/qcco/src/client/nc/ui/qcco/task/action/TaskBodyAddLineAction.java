				...
				if (custlist != null && custlist.size() > 0) {
					for (Map<String, String> map : custlist) {
						...
						for (Entry<String, String> refValue : map.entrySet()) {
							...
							} else if (refValue.getKey().equals("valueways")) {
								if (map.get("pk_list_table") != null) {
									this.getGrandCard().getBillCardPanel()
											.setBodyValueAt(map.get("pk_list_table"), row, "valueways", "pk_task_s");
								}
							}
						}
						...
						// this.getGrandCard().getBillCardPanel().setBodyValueAt(UFBoolean.TRUE,
						// row, "conditionstatus", "pk_task_s");
