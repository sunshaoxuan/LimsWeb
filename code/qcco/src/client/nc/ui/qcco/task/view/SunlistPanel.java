...
public class SunlistPanel extends UIDialog implements
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(getOkButton())) {
			...
			if (bodySelectedVOs != null && bodySelectedVOs.length > 0) {
				for (TaskBodyVO taskBodyVO : bodySelectedVOs) {
					getPklist().add(taskBodyVO.getTestList());
				}
			}
			dispose();
	...
	private void onButtonOKtopClicked() {
		...
		// 查询出listData
		List<TaskBodyVO> taskvos = new ArrayList<>();
