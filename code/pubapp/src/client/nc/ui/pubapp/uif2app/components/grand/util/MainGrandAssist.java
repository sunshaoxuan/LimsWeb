...
public class MainGrandAssist {
    ...
    public List<Object> getGrandListDataByMainRow(BillListView mainlistview, int currentRow,
        ...
        if (currentRow != -1) {
            SuperVO bodyVO =
                    (SuperVO) mainlistview.getBillListPanel().getBodyBillModel()
    ...
    public void updateGrandListByBodyTab(ListGrandPanelComposite listcombo) {
        ...
        String currentBodyTab =
                ((BillListView) listcombo.getMainPanel()).getBillListPanel().getChildListPanel().getTableCode();
        Component grandComposite =
            (Component) listcombo.getMaingrandrelationship().getBodyTabTOGrandListComposite().get(currentBodyTab);
	...
	public void updateGrandCardByBodyTab(CardGrandPanelComposite cardcombo) {
        ...
        // 分割线当前位置
        Component grandComposite =
            (Component) cardcombo.getMaingrandrelationship().getBodyTabTOGrandCardComposite().get(currentBodyTab);
    ...
    public List<Object> getGrandCardDataByMainRow(RowChangeBean rowChangeBean,
        ...
        /*if (pk_body == null) {
            String precolum = RowNumUtil.getRowNumUtil().generateRowNo();
            this.addPreColumBillItemToBody(mainbillform, currentRow, bodyTabCode, precolum);
            currBodyVO.setAttributeValue("precolumn", precolum);
    ...
    public Map<String, ArrayList<Object>> getGrandCardDataByMainRowAdd(RowChangeBean rowChangeBean,
    	...
    	String pk_body="";
		try {
			CircularlyAccessibleValueObject bodyValueRowVO = mainbillform.getBillCardPanel().getBillModel().getBodyValueRowVO(currentRow, className);
			if(null!=bodyTabCode){
    ...
    public void showMainCardDataByGrandTab(BillForm mainBillForm, int currentRow,
        ...
        if (mainBillForm.getModel().getUiState().equals(UIState.ADD)) {
            ...
            Component grandComposite =
                    (Component) maingrandrelationship.getBodyTabTOGrandCardComposite().get(currentBodyTabCode);
            if (grandComposite == null) {
            	return;
            }
            String[] currentGrandTabCodes =
                    ((BillForm) grandComposite).getBillCardPanel().getBillData().getBodyTableCodes();
            String pk_body =
                    (String) mainBillForm.getBillCardPanel().getBillModel().getValueAt(currentRow, "precolumn");
            for (String currentGrandTabCode : currentGrandTabCodes) {
                ...
                CircularlyAccessibleValueObject[] grandVos =
                        MainGrandUtil.getBodyVOsByTabCode((BillForm) grandComposite, currentGrandTabCode);
    ...
    public void showMainCardDataByGrandTabEdit(BillForm mainBillForm, int currentRow,
        ...
        if (mainBillForm.getModel().getUiState().equals(UIState.EDIT)) {
            ...
            CircularlyAccessibleValueObject[] childrenVOs =
                    MainGrandUtil.getBodyVOsByTabCode(mainBillForm, currentBodyTabCode);
    ...
    public void delGrandCardDataByMainRowEdit(BillForm mainbillform, int deleline, BillForm grandbillform) {
        ...
        if (!ArrayUtils.isEmpty(tabCodes)) {
            for (String grandTabCode : tabCodes) {
                ...
                ArrayList<Object> delGrandVOList =
                        this.getMainGrandModel().getBufferCardEditMap().get(lastcardUniqueKey);
    ...
    private void putDataIntoAddMap(RowChangeBean rowChangeBean, MainGrandRelationShip maingrandrelationship,
        ...
        if (lastrow >= 0) {
			...
			try {
				CircularlyAccessibleValueObject bodyValueRowVO = mainbillform.getBillCardPanel().getBillModel().getBodyValueRowVO(lastrow, className);
				if(null!=bodyTabCode){
    ...
    private void putDataIntoAddMapForCurrentRow(RowChangeBean rowChangeBean, MainGrandRelationShip maingrandrelationship) {
        ...
        if (currentRow >= 0) {
            ...
            if (last_pk_body == null) {
                this.addPreColumBillItemToBody(mainbillform, currentRow, bodyTabCode, RowNumUtil.getRowNumUtil()
                        .generateRowNo());
                last_pk_body = (String) mainbillform.getBillCardPanel().getBillModel().getValueAt(currentRow, "precolumn");
            }
            String lastcardUniqueKey = "";
            if (!ArrayUtils.isEmpty(tabCodes)) {
                for (String grandTabCode : tabCodes) {
        ...
        if (lastrow >= 0) {
        	...
        	for(String grandTabCode : grandTabCodes){
        		...
        		if(grandRowNumList != null && grandRowNumList.size() > 0){
        			...
        			CircularlyAccessibleValueObject[] grandVOs = MainGrandUtil.getBodyVOsByTabCode(grandbillform, grandTabCode);
        			ArrayList<Object> lastGrandVosList = new ArrayList<Object>();
    ...
    private void processDelVOs(BillForm grandbillform, String grandTabCode, String cardUniqueKey) {
        ...
        CircularlyAccessibleValueObject[] lastDeleteGrandVos =
                MainGrandUtil.getBodyChangeVoByTabCode(grandbillform, grandTabCode);
    ...
    public ArrayList<Object> setGrand2CurrentPanel(CardGrandPanelComposite mainPanel) {
        ...
        BillForm grandBillForm =
                (BillForm) mainPanel.getMaingrandrelationship().getBodyTabTOGrandCardComposite()
        ...
        if (grandBillForm != null) {
            ...
            if(className != null){
            	childBodyVos = mainBillForm.getBillCardPanel().getBillModel(currentbodyTabCode).getBodyValueVOs(className);
            }
            if(childBodyVos == null || childBodyVos.length == 0){
            	return null;
            }
    ...
    /**
     * 编辑态获得孙表数据
     * <li>当编辑缓存中有孙表数据，然后检查删除缓存中时候有，有则删除</li>
     * <li>当编辑缓存中没有孙表数据，检查删除缓存，如果删除缓存中有，则为空</li>
     * <li>当编辑缓存中没有孙表数据，删除缓存中也没有，则直接从查询缓存中取</li>
     */
	private ArrayList<Object> getGrandListStatusIsEdit(String cardUniqueKey) {
        ...
        if (grandObjectList != null && grandObjectList.size() != 0) {
            ...
            if (delList != null && delList.size() != 0) {
                grandObjectList =
                        MainGrandUtil.processList(grandObjectList,
    ...
    public void setGrandStatusByMainForm(CardGrandPanelComposite zhuzisuncard){
    	...
    	for(String tabCode : tabCodes){
    		BillForm grandForm = (BillForm) zhuzisuncard.getMaingrandrelationship().
    								getBodyTabTOGrandCardComposite().get(tabCode);
    		if(grandForm != null){
