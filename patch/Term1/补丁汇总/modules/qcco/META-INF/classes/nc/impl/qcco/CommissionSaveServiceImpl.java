...
public class CommissionSaveServiceImpl implements ICommissionSaveService{
	...
	public IBill[] save(String stat, Object value, List<CommissionRVO> list) throws BusinessException{
		...
		if(stat.equals("add")){
			return NCLocator.getInstance().lookup(IQccommissionMaintain.class).insert(new AggCommissionHVO[]{(AggCommissionHVO) value});
		}
		if(stat.equals("edit")){
			return NCLocator.getInstance().lookup(IQccommissionMaintain.class).update(new AggCommissionHVO[]{(AggCommissionHVO)value});
		}
		return null;
